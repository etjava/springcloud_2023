...
export default {
  ...
  methods:{
    getSrcUrl(t){
      return genServerUrl('image/'+t,"image");
    },
    // 刷新验证码
    refreshVerifyCode(){
      let imgUrl = genServerUrl("get","verifyCode");
    ...
    exec_miaosha(){
      ...
      let url=genServerUrl("exec","miaoSha");
    ...
    getMiaoShaResult(miaoShaGoodsId){
      let url=genServerUrl("result","miaoSha");
      let token=window.sessionStorage.getItem("token");
      axios.defaults.headers.common['token']=token;
      axios.get(url,{
        params:{
    ...
    getInfo(){
      let url=genServerUrl("findById","miaoShaGoods");
      let token=window.sessionStorage.getItem("token");
      axios.defaults.headers.common['token']=token;
